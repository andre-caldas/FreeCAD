    ...
    : Gui::MDIView(doc, parent), m_vpPage(pageVp)
{
    ...
    QString tabText = QString::fromUtf8(pageVp->getDrawPage()->_getNameInDocument());
...
void MDIViewPage::onDeleteObject(const App::DocumentObject& obj)
{
    ...
    if (obj.isDerivedFrom(TechDraw::DrawView::getClassTypeId())) {
        (void)m_scene->removeQViewByName(obj._getNameInDocument());
    }
    blockSceneSelection(false);
}
...
void MDIViewPage::preSelectionChanged(const QPoint& pos)
{
    ...
    if (edge) {
        ...
        Gui::Selection().setPreselect(viewObj->getDocument()->getName(),
                                      viewObj->_getNameInDocument(), ss.str().c_str(), pos.x(),
                                      pos.y(), 0);
    }
}
        ...
        if (!itemView) {
            ...
            if (edge) {
                ...
                static_cast<void>(Gui::Selection().addSelection(viewObj->getDocument()->getName(),
                                                                viewObj->_getNameInDocument(),
                                                                ss.str().c_str()));
                showStatusMsg(viewObj->getDocument()->getName(), viewObj->_getNameInDocument(),
                              ss.str().c_str());
                continue;
            }
            ...
            if (mText) {
                ...
                static_cast<void>(Gui::Selection().addSelection(
                    parentFeat->getDocument()->getName(), parentFeat->_getNameInDocument()));
            }
        }
        else {
