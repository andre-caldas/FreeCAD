    ...
    if (parametric) {
        ...
        std::for_each(objs.begin(), objs.end(), [](App::DocumentObject* obj) {
            try {
                ...
                copyVisual("ActiveObject", "ShapeColor", obj->_getNameInDocument());
                copyVisual("ActiveObject", "LineColor", obj->_getNameInDocument());
                copyVisual("ActiveObject", "PointColor", obj->_getNameInDocument());
            }
