    ...
    if (!filter) {
        ...
        try{
            ...
            for (App::DocumentObject* obj: sources){
                ...
                features_to_hide.append(QString::fromLatin1(obj->_getNameInDocument()));
...
void DlgExtrusion::findShapes()
{
    ...
    for (auto obj : objs) {
        ...
        if (canExtrude(shape)) {
            ...
            item->setData(0, Qt::UserRole, QString::fromLatin1(obj->_getNameInDocument()));
...
void DlgExtrusion::setAxisLink(const App::PropertyLinkSub& lnk)
{
    ...
    if (lnk.getSubValues().size() == 1){
        this->setAxisLink(lnk.getValue()->_getNameInDocument(), lnk.getSubValues()[0].c_str());
    } else {
        this->setAxisLink(lnk.getValue()->_getNameInDocument(), "");
    }
}
...
{
    Gui::Command::doCommand(Gui::Command::Doc,"f = App.getDocument('%s').getObject('%s')", feature.getDocument()->getName(), feature._getNameInDocument());

    if (base)
        Gui::Command::doCommand(Gui::Command::Doc,"f.Base = App.getDocument('%s').getObject('%s')", base->getDocument()->getName(), base->_getNameInDocument());
