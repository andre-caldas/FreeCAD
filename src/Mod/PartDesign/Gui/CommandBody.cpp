...
void CmdPartDesignBody::activated(int iMsg)
{
    ...
    if (baseFeature) {
        if (partOfBaseFeature){
            ...
            doCommand(Doc,"App.activeDocument().%s.removeObject(App.activeDocument().%s)",
                    partOfBaseFeature->_getNameInDocument(), baseFeature->_getNameInDocument());
        }
        if (addtogroup) {
            doCommand(Doc,"App.activeDocument().%s.Group = [App.activeDocument().%s]",
                    bodyString, baseFeature->_getNameInDocument());
        }
        else {
    ...
    if (actPart) {
        doCommand(Doc,"App.activeDocument().%s.addObject(App.ActiveDocument.%s)",
                 actPart->_getNameInDocument(), bodyString);
    }
...
void CmdPartDesignMigrate::activated(int iMsg)
{
    ...
    for ( auto chainIt = featureChains.begin(); !featureChains.empty();
        ...
        doCommand ( Doc,"App.activeDocument().%s.addObject(App.ActiveDocument.%s)",
                actPart->_getNameInDocument(), bodyName.c_str() );
        if (base) {
            doCommand ( Doc,"App.activeDocument().%s.BaseFeature = App.activeDocument().%s",
                bodyName.c_str(), base->_getNameInDocument() );
        }
        ...
        for ( auto feature: *chainIt ) {
            if ( feature->isDerivedFrom ( PartDesign::ProfileBased::getClassTypeId() ) ) {
                ...
                if ( sketch ) {
                    doCommand ( Doc,"App.activeDocument().%s.addObject(App.activeDocument().%s)",
                            bodyName.c_str(), sketch->_getNameInDocument() );
            ...
            doCommand ( Doc,"App.activeDocument().%s.addObject(App.activeDocument().%s)",
                    bodyName.c_str(), feature->_getNameInDocument() );
...
void CmdPartDesignMoveFeature::activated(int iMsg)
{
    ...
    /*
        ...
        if (source) {
            ...
            doCommand(Doc,"App.activeDocument().%s.removeObject(App.activeDocument().%s)",
                      source->_getNameInDocument(), (feat)->_getNameInDocument());
        }
        ...
        doCommand(Doc,"App.activeDocument().%s.addObject(App.activeDocument().%s)",
                      target->_getNameInDocument(), (feat)->_getNameInDocument());
        ...
        if ( featureWasTip ) {
            ...
            if (sourceNewTip)
                doCommand(Gui,"Gui.activeDocument().show(\"%s\")", sourceNewTip->_getNameInDocument());
        }
        ...
        if ( targetOldTip != targetNewTip ) {
            if ( targetOldTip ) {
                doCommand(Gui,"Gui.activeDocument().hide(\"%s\")", targetOldTip->_getNameInDocument());
            }
            if (targetNewTip) {
                doCommand(Gui,"Gui.activeDocument().show(\"%s\")", targetNewTip->_getNameInDocument());
            }
        }
