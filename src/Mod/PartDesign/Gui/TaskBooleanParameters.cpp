    ...
    , BooleanView(BooleanView)
{
    ...
    for (auto body : bodies) {
        ...
        item->setData(Qt::UserRole, QString::fromLatin1(body->_getNameInDocument()));
    }
    ...
    if (msg.Type == Gui::SelectionChanges::AddSelection) {
        ...
        if (selectionMode == bodyAdd) {
            if (std::find(bodies.begin(), bodies.end(), pcBody) == bodies.end()) {
                ...
                item->setData(Qt::UserRole, QString::fromLatin1(pcBody->_getNameInDocument()));
...
void TaskBooleanParameters::onButtonBodyAdd(bool checked)
{
    if (checked) {
        ...
        if (pcBoolean->Group.getValues().empty() && pcBoolean->BaseFeature.getValue())
            doc->setHide(pcBoolean->BaseFeature.getValue()->_getNameInDocument());
...
void TaskBooleanParameters::onBodyDeleted()
{
    ...
    for (auto it = bodies.begin(); it != bodies.end(); ++it) {
        if (internalName == QLatin1String((*it)->_getNameInDocument())) {
            body = *it;
            bodies.erase(it);
            break;
        }
...
void TaskBooleanParameters::exitSelectionMode()
{
    ...
    Gui::Document* doc = Gui::Application::Instance->activeDocument();
    if (doc)
        doc->setShow(BooleanView->getObject()->_getNameInDocument());
}
...
bool TaskDlgBooleanParameters::reject()
{
    ...
    if (doc) {
        if (obj->BaseFeature.getValue()) {
            doc->setShow(obj->BaseFeature.getValue()->_getNameInDocument());
            std::vector<App::DocumentObject*> bodies = obj->Group.getValues();
            for (auto body : bodies) {
                doc->setShow(body->_getNameInDocument());
            }
        }
    }
