...
void CmdRobotEdge2Trac::activated(int)
{
    ...
    if (ObjectFilter.match()) {
        ...
        doCommand(Gui, "Gui.activeDocument().setEdit('%s')", EdgeObj->_getNameInDocument());
    }
...
void CmdRobotTrajectoryDressUp::activated(int)
{
    ...
    if (ObjectFilterDressUp.match()) {
        ...
        doCommand(Gui, "Gui.activeDocument().setEdit('%s')", Object->_getNameInDocument());
    }
        ...
        doCommand(Gui,
                  ...
                  FeatName.c_str(),
                  Object->_getNameInDocument());
        doCommand(Gui, "Gui.activeDocument().hide(\"%s\")", Object->_getNameInDocument());
        doCommand(Gui, "Gui.activeDocument().setEdit('%s')", FeatName.c_str());
    }
    else {
        QMessageBox::warning(Gui::getMainWindow(),
