...
App.activeDocument().%s",base->_getNameInDocument());
...
void CmdFemCreateNodesSet::activated(int)
{
    ...
    if (ObjectFilter.match()) {
        ...
        doCommand(Gui, "Gui.activeDocument().setEdit('%s')", NodesObj->_getNameInDocument());
    }
