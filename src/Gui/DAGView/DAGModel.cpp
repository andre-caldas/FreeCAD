...
void Model::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
  auto goShiftSelect = [this, event]()
  {
    ...
    for (auto currentItem : selection)
    {
      ...
      Gui::Selection().addSelection(selectionRecord.DObject->getDocument()->getName(),
                                    selectionRecord.DObject->_getNameInDocument());
    }
  };
  ...
  auto toggleSelect = [](const App::DocumentObject *dObjectIn, RectItem *rectIn)
  {
    if (rectIn->isSelected())
      Gui::Selection().rmvSelection(dObjectIn->getDocument()->getName(), dObjectIn->_getNameInDocument());
    else
      Gui::Selection().addSelection(dObjectIn->getDocument()->getName(), dObjectIn->_getNameInDocument());
  };
    ...
    if (rect)
    {
        ...
        if (selectionMode == SelectionMode::Single)
        {
          ...
          else
          {
            ...
            Gui::Selection().addSelection(dObject->getDocument()->getName(), dObject->_getNameInDocument());
          }
        }
...
void Model::contextMenuEvent(QGraphicsSceneContextMenuEvent* event)
{
  ...
  if (rect)
  {
    ...
    if (!rect->isSelected())
    {
      ...
      Gui::Selection().addSelection(record.DObject->getDocument()->getName(), record.DObject->_getNameInDocument());
      lastPickValid = true;
      lastPick = event->scenePos();
    }
